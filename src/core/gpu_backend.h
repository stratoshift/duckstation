...
public:
  ...
  // NOTE: Only safe to call on GPU thread.

  /// Returns the full display resolution of the GPU, including padding.
  static std::tuple<u32, u32> GetDisplaySize();
  static std::tuple<u32, u32> GetDisplayVRAMSize();
  static float GetDisplayAspectRatio();

  /// Returns the current resolution scale.
  static u32 GetResolutionScale();
...
public:
  ...
  ALWAYS_INLINE s32 GetDisplayViewWidth() { return m_display_texture_view_width; }
  ALWAYS_INLINE s32 GetDisplayViewHeight() { return m_display_texture_view_height; }
...
protected:
  ...
  void SetDisplayTexture(GPUTexture* texture, GPUTexture* depth_buffer, s32 view_x, s32 view_y, s32 view_width,
                         s32 view_height);
  void SetResolutionScaleStat(u32 resolution_scale);
