...
static s32 s_display_width = 0;
static s32 s_display_height = 0;
static s32 s_display_origin_left = 0;
static s32 s_display_origin_top = 0;
static s32 s_display_vram_width = 0;
static s32 s_display_vram_height = 0;
static float s_display_aspect_ratio = 0.0f;
static u32 s_resolution_scale_stat = 0;
...
GPUBackend::GPUBackend()
{
  ...
  s_display_width = 0;
  s_display_height = 0;
  s_display_origin_left = 0;
  s_display_origin_top = 0;
  s_display_vram_width = 0;
  s_display_vram_height = 0;
  s_display_aspect_ratio = 1.0f;
}
...
std::tuple<u32, u32> GPUBackend::GetDisplaySize()
{
  return std::tie(s_display_width, s_display_height);
}

std::tuple<u32, u32> GPUBackend::GetDisplayVRAMSize()
{
  return std::tie(s_display_vram_width, s_display_vram_height);
}

float GPUBackend::GetDisplayAspectRatio()
{
  return s_display_aspect_ratio;
}

u32 GPUBackend::GetResolutionScale()
{
  return s_resolution_scale_stat;
}
...
void GPUBackend::HandleCommand(const GPUThreadCommand* cmd)
{
  switch (cmd->type)
  {
    ...
    case GPUBackendCommandType::UpdateDisplay:
    {
      ...
      s_display_width = ccmd->display_width;
      s_display_height = ccmd->display_height;
      s_display_origin_left = ccmd->display_origin_left;
      s_display_origin_top = ccmd->display_origin_top;
      s_display_vram_width = ccmd->display_vram_width;
      s_display_vram_height = ccmd->display_vram_height;
      s_display_aspect_ratio = ccmd->display_aspect_ratio;
...
void GPUBackend::SetResolutionScaleStat(u32 resolution_scale)
{
  s_resolution_scale_stat = resolution_scale;
                                                   ...
                                                   const GSVector4i draw_rect, bool postfx)
{
                                                 ...
                                                 display_texture_view_height))
  {
    ...
    if (const GPUDevice::PresentResult pres = PostProcessing::InternalChain.Apply(
          ...
          display_texture_view_height, s_display_width, s_display_height);
  ...
  if (really_postfx)
  {
    ...
    const float upscale_x =
      m_display_texture ? static_cast<float>(m_display_texture_view_width) / static_cast<float>(s_display_vram_width) :
                          1.0f;
    const float upscale_y = m_display_texture ? static_cast<float>(m_display_texture_view_height) /
                                                  static_cast<float>(s_display_vram_height) :
                                                1.0f;
    const s32 orig_width = static_cast<s32>(std::ceil(static_cast<float>(s_display_width) * upscale_x));
    const s32 orig_height = static_cast<s32>(std::ceil(static_cast<float>(s_display_height) * upscale_y));

    return PostProcessing::DisplayChain.Apply(PostProcessing::DisplayChain.GetInputTexture(), nullptr, target,
                                              display_rect, orig_width, orig_height, s_display_width, s_display_height);
  }
  else
  {
                                   ...
                                   GSVector4i* display_rect, GSVector4i* draw_rect) const
{
  ...
  const float display_aspect_ratio = s_display_aspect_ratio;
  const float window_ratio = static_cast<float>(window_width) / static_cast<float>(window_height);
  const float crtc_display_width = static_cast<float>(show_vram ? VRAM_WIDTH : s_display_width);
  const float crtc_display_height = static_cast<float>(show_vram ? VRAM_HEIGHT : s_display_height);
  const float x_scale =
    apply_aspect_ratio ?
  ...
  float active_left = static_cast<float>(show_vram ? 0 : s_display_origin_left);
  float active_top = static_cast<float>(show_vram ? 0 : s_display_origin_top);
  float active_width = static_cast<float>(show_vram ? VRAM_WIDTH : s_display_vram_width);
  float active_height = static_cast<float>(show_vram ? VRAM_HEIGHT : s_display_vram_height);
  ...
  if (g_gpu_settings.display_rotation == DisplayRotation::Rotate90 ||
